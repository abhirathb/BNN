...
maf = sys.argv[1]
hc = sys.argv[2]
X = np.loadtxt(maf) #importing MAF for given dataset
...
n_preds = X.shape[1]
print n_preds
hidden_layer = Sigmoid_Layer(n_units=nh,n_incoming=n_preds,N=N) #defining a sigmoid hidden layer with 5 nodes and capacity to take from n_preds input variables and a total of N subjects
...
output_layer = Softmax_Layer(n_classes=n_classes,n_incoming=nh,N=N)
...
net.initialize(iters=10000,verbose=True,step_size=1e-5,include_prior=True) 


#hmc = HMC_sampler(net,L=15,eps=0.001)
